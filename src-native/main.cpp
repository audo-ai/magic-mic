...
void quit_tray_cb(struct tray_menu *menu) { *(bool *)menu->context = true; }
void open_tray_cb(struct tray_menu *menu) { *(bool *)menu->context = true; }
int main(int argc, char **argv) {
  ...
  struct tray_menu menu[] = {
      {quit_text, 0, 0, quit_tray_cb, &tray_exit_requested},
  ...
  while (running) {
    if (std::future_status::ready ==
        err_fut.wait_for(std::chrono::milliseconds(0))) {
      try {
    ...
    ret = pselect(1 + (sock_fd == -1 ? serv_fd : sock_fd), &rfds, nullptr,
                  nullptr, &tv, &mask);
    tray_loop(0);
    ...
    if (FD_ISSET(serv_fd, &rfds)) {
      if (sock_fd != -1) {
        ...
        if (-1 == (sock_fd = accept(serv_fd, nullptr, nullptr))) {
          logger->debug("Error accepting connection (even though we will "
                        "imediately close): {}",
                        strerror(errno));
        } else {
          close(sock_fd);
